...
class Generator(nn.Module):
    ...
    def __init__(self, ngf=64, nz=100, nc=3, im_size=512, sle=True, big=False, \
                infogan_latent_dim=0, infogan_cat_size=None, spatial_code_dim=0):
        ...
        nz += infogan_latent_dim
        nz += spatial_infogan_size*spatial_code_dim
        if infogan_cat_size != None:
            nz += infogan_cat_size[0] * infogan_cat_size[1]
...
class Discriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512, sle=True, decode=True, big=False, \
        ...
        if infogan_cat_size != None:
            ...
            self.infogan_n_classes = infogan_cat_size[1]
        ...
        if big:
            self.down_4  = DownBlockComp(ndf//4, ndf//2)
            self.down_8  = DownBlockComp(ndf//2, ndf*1)
            self.down_16 = DownBlockComp(ndf*1,  ndf*2)
            self.down_32 = DownBlockComp(ndf*2,  ndf*4)
            self.down_64 = DownBlockComp(ndf*4,  ndf*16)
        else:    
        ...
        self.rf_big = nn.Sequential(
                            conv2d(ndf*16 , ndf*16, 1, 1, 0, bias=False), # 8x8
                            batchNorm2d(ndf*16),
                            nn.LeakyReLU(0.2, inplace=True),
                            conv2d(ndf * 16, 1, 4, 1, 0, bias=False)) # 5x5
        ...
        self.rf_from_128 = conv2d(ndf*4, 1, 4, 1, 0, bias=False) # rf: real/fake
        ...
        if self.infogan_latent_dim:
            # predict from feat_small 256x8x8, spectral norm?
            self.latent_from_128 = nn.Sequential(DownBlock(ndf*4, ndf*4), # 256x8x8 -> 256x4x4
                                                conv2d(ndf*4, 128, 4, bias=False), # 128x1x1
                                                batchNorm2d(128),
    ...
    def forward(self, imgs, label):
        ...
        feat_small = self.down_from_small(imgs[1]) # imgs[1]: input images resized to 128x128
        rf_1 = self.rf_from_128(feat_small)

        if self.infogan_latent_dim:
            y = self.latent_from_128(feat_small)
        ...
        if label=='real' and self.decode:    
            ...
            rec_img_small = self.decoder_small(feat_small) # reconstruct small img

            part = random.randint(0, 3) # reconstruct part img
            rec_img_part = None
            if part==0:
                rec_img_part = self.decoder_part(feat_32[:,:,:8,:8]) # feat_32 size 16x16, crop to 8x8, decode to 128x128
            if part==1:
        ...
        if self.infogan_latent_dim and not self.spatial_code_dim:
    ...
    def get_feat(self, img):
        ...
        feat_last = self.se_8_64(feat_8, feat_last)
        return self.rf_big[0](feat_last)
